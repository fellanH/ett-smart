document.addEventListener("DOMContentLoaded", () => {
  ...
  function switchView(viewId) {
    ...
    [btnDashboard, btnUpload, btnTerminal].forEach((btn) =>
  ...
  function generateStats() {
    ...
    const turnoverCol = headers.find(
      (h) =>
        h.toLowerCase().includes("omsättning") ||
        h.toLowerCase().includes("turnover"),
    );

    if (statusCol) {
      const activeCount = currentData.filter(
        (r) => r[statusCol] === "Aktiv",
      ).length;
          ...
          headers
            .map((h) => {
              ...
              if (h.toLowerCase().includes("status")) {
                if (val === "Aktiv")
                  return `<td><span class="status-badge status-active">${val}</span></td>`;
  ...
  async function executeCommand() {
    ...
    try {
      ...
      if (result.error)
        appendOutput(`Execution Error: ${result.error}`, "error");
    } catch (err) {
